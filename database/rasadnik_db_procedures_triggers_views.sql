...
begin
	if (new.paid_off=true) then
		insert into transaction values (null, new.price, true, new.purchase_id, false); -- false je in, a true je out
	end if;
end$$
delimiter ;
...
begin
	if (new.paid_off=true) then
		insert into transaction values (null, new.price, false, new.sale_id, false); -- false je in, a true je out
	end if;
end$$
delimiter ;
